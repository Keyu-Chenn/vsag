#!/usr/bin/env python3
"""Prepare uniform per-query alphas and their exact hybrid ground truth."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate uniform per-query alphas and exact mixed-alpha ground truth.",
...
def make_uniform_alphas(count: int, alpha_min: float, alpha_max: float, seed: int) -> np.ndarray:
    quantiles = (np.arange(count, dtype=np.float64) + 0.5) / count
    alphas = alpha_min + (alpha_max - alpha_min) * quantiles
...
def main() -> None:
    ...
    with h5py.File(args.hdf5, "r") as h5:
        ...
        alphas = make_uniform_alphas(
            num_queries, args.alpha_min, args.alpha_max, args.seed
        )
        ...
        print(
            f"uniform alpha: min={alphas.min():.6f}, max={alphas.max():.6f}, "
            f"mean={alphas.mean():.6f}, seed={args.seed}"
        )
        ...
        for chunk_idx, train_start in enumerate(
            ...
            for query_start, query_end in query_ranges(num_queries, args.query_chunk):
                ...
                scores = query_alphas * dense_scores + (1.0 - query_alphas) * sparse_scores
