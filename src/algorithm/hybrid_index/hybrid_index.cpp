...
HybridIndex::HybridIndex(const HybridIndexParameterPtr& param, const IndexCommonParam& common_param)
    ...
    inner_codes_ = FlattenInterface::MakeInstance(param->base_codes_param, common_param); // get params
...
HybridIndex::CheckAndMappingExternalParam(const JsonType& external_param,
    ...
    auto brute_force_parameter = std::make_shared<HybridIndexParameter>();
...
std::vector<int64_t>
HybridIndex::Build(const vsag::DatasetPtr& data) {
    this->Train(data);
...
uint64_t
HybridIndex::EstimateMemory(uint64_t num_elements) const {
    return num_elements *
           (this->dim_ * sizeof(float) + sizeof(LabelType) * 2 + sizeof(InnerIdType));
}

bool
HybridIndex::Remove(int64_t label) {
...
HybridIndex::SearchWithRequest(const SearchRequest& request) const {
    ...
    auto brute_force_params = HybridIndexSearchParameter::FromJson(request.params_str_);
...
void
HybridIndex::resize(uint64_t new_size) {
...
void
HybridIndex::add_one(const float* data, InnerIdType inner_id) {
...
void
HybridIndex::UpdateAttribute(int64_t id, const AttributeSet& new_attrs) {
    auto inner_id = this->label_table_->GetIdByLabel(id);
    this->attr_filter_index_->UpdateBitsetsByAttr(new_attrs, inner_id, 0);
}

void
HybridIndex::UpdateAttribute(int64_t id,
