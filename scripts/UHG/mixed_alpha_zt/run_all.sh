#!/usr/bin/env bash
# Serially run truncated-normal mixed-alpha Recall@10/@20/@50/@100/@200 for
# the selected datasets and five methods, keeping at most one experiment
# hybrid index. MSMARCO is supported but excluded from the default selection.
#
# Usage:
#   bash /tbase-project/vsag/scripts/UHG/mixed_alpha_zt/run_all.sh
#   DRY_RUN=1 bash /tbase-project/vsag/scripts/UHG/mixed_alpha_zt/run_all.sh
set -Eeuo pipefail
...
INDEX_DIR="${INDEX_DIR:-$SHARED_DATA_DIR/index}"
INDEX_HYBRID_DIR="${INDEX_HYBRID_DIR:-$SHARED_DATA_DIR/index_hybrid_union}"
RESULT_ROOT="${RESULT_ROOT:-$EXPERIMENT_ROOT/results}"
INDEX_SIZE_REFERENCE="${INDEX_SIZE_REFERENCE:-$REPO_ROOT/scripts/UHG/results/results_integration/construction/index_size.txt}"

BIN_HYBRID="${BIN_HYBRID:-$REPO_ROOT/build-release/examples/cpp/703_uhg_exp3}"
DATASETS_TEXT="${DATASETS_TEXT:-nq hotpotqa dbpedia-entity fever}"
RECALLS_TEXT="${RECALLS_TEXT:-10 20 50 100 200}"
METHODS_TEXT="${METHODS_TEXT:-hnsw sindi hnsw_sindi fhg uhg}"
NUM_QUERIES="${NUM_QUERIES:--1}"
THREADS="${THREADS:-1}"
RESUME="${RESUME:-1}"
DRY_RUN="${DRY_RUN:-0}"
HYBRID_DISK_RESERVE_GIB="${HYBRID_DISK_RESERVE_GIB:-2}"
...
CANONICAL_SHARED_DATA_DIR="/tbase-project/vsag/scripts/UHG/data"
...
ALL_ALLOWED_DATASETS=(nq hotpotqa msmarco dbpedia-entity fever)
...
hybrid_index_size_gib() {
    local dataset="$1" kind="$2" key column
    case "$dataset" in
        nq) key="NQ" ;;
        hotpotqa) key="HotpotQA" ;;
        dbpedia-entity) key="dbpedia-entity" ;;
        fever) key="fever" ;;
        msmarco)
            if [ "$kind" = "uhg" ]; then
                column=4
            ...
            awk -v column="$column" \
                '$1 == "MS" && $2 == "MARCO" { print $column; exit }' \
                "$INDEX_SIZE_REFERENCE"
            return
            ;;
        *)
            echo "Unsupported dataset for disk estimate: $dataset" >&2
            return 2
            ;;
    esac
    if [ "$kind" = "uhg" ]; then
    ...
    awk -v key="$key" -v column="$column" \
        '$1 == key { print $column; exit }' \
        "$INDEX_SIZE_REFERENCE"
}

check_hybrid_disk_space() {
    local dataset="$1" kind="$2"
    local expected_gib expected_bytes reserve_bytes required_bytes available_bytes
    expected_gib="$(hybrid_index_size_gib "$dataset" "$kind")"
    if ! [[ "$expected_gib" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
        echo "Cannot read $kind index-size estimate for $dataset from $INDEX_SIZE_REFERENCE" >&2
        return 2
    fi
    expected_bytes="$(awk -v gib="$expected_gib" \
        'BEGIN { printf "%.0f", gib * 1024 * 1024 * 1024 }')"
    reserve_bytes="$(awk -v gib="$HYBRID_DISK_RESERVE_GIB" \
        'BEGIN { printf "%.0f", gib * 1024 * 1024 * 1024 }')"
    required_bytes="$((expected_bytes + reserve_bytes))"
    available_bytes="$(df -PB1 "$SHARED_DATA_DIR" | awk 'NR == 2 { print $4 }')"
    awk -v dataset="$dataset" -v kind="$kind" \
        -v available="$available_bytes" -v expected="$expected_bytes" \
        -v reserve="$reserve_bytes" \
        'BEGIN {
            printf "Hybrid disk precheck: dataset=%s kind=%s available=%.2f GiB expected=%.2f GiB reserve=%.2f GiB\n",
                dataset, kind, available / 2^30, expected / 2^30, reserve / 2^30
        }'
    if [ "$available_bytes" -lt "$required_bytes" ]; then
        echo "Insufficient disk for $dataset $kind hybrid index; refusing to build" >&2
        return 1
    fi
}
...
hybrid_index_path() {
    ...
    case "$kind" in
        fhg)
            printf '%s/index/703_%s_fhg_hybrid_index.index' \
                "$SHARED_DATA_DIR" "$dataset"
            ;;
        uhg)
            printf '%s/index_hybrid_union/703_%s_hybrid_index.index' \
                "$SHARED_DATA_DIR" "$dataset"
            ;;
        *)
...
initialize_virtual_index_state() {
    ...
    for dataset in "${ALL_ALLOWED_DATASETS[@]}"; do
...
validate_deletion_target() {
    ...
    if [ "$SHARED_DATA_DIR" != "$CANONICAL_SHARED_DATA_DIR" ]; then
        echo "Refusing hybrid deletion with non-canonical SHARED_DATA_DIR: $SHARED_DATA_DIR" >&2
        return 2
    fi
    case "$kind" in
        fhg)
            expected="$CANONICAL_SHARED_DATA_DIR/index/703_${dataset}_fhg_hybrid_index.index"
            ;;
        uhg)
            expected="$CANONICAL_SHARED_DATA_DIR/index_hybrid_union/703_${dataset}_hybrid_index.index"
            ;;
        *) return 2 ;;
    esac
...
delete_hybrid_index() {
    ...
    if ! is_allowed_dataset "$dataset"; then
        echo "Refusing hybrid deletion for unsupported dataset: $dataset" >&2
        return 2
    fi
    path="$(hybrid_index_path "$dataset" "$kind")"
...
preflight() {
    ...
    require_file "$INDEX_SIZE_REFERENCE"
...
build_fhg_index() {
    ...
    check_hybrid_disk_space "$dataset" fhg
    h5="$SHARED_DATA_DIR/hdf5/${dataset}.hdf5"
    graph="$SHARED_DATA_DIR/fhg/${dataset}_fhg_alpha_0_5.h5"
    alpha_file="$EXPERIMENT_ROOT/data/gt_top200/${dataset}/alphas.npy"
    gt_file="$EXPERIMENT_ROOT/data/gt_top200/${dataset}/ground_truth.npy"
...
build_uhg_index() {
    ...
    check_hybrid_disk_space "$dataset" uhg
    h5="$SHARED_DATA_DIR/hdf5/${dataset}.hdf5"
    graph="$SHARED_DATA_DIR/uhg/${dataset}_uhg.h5"
...
ensure_only_target_hybrid_index() {
    local target_dataset="$1" target_kind="$2"
    local dataset kind path target_path
    target_path="$(hybrid_index_path "$target_dataset" "$target_kind")"
    for dataset in "${ALL_ALLOWED_DATASETS[@]}"; do
        for kind in fhg uhg; do
            path="$(hybrid_index_path "$dataset" "$kind")"
            if [ "$path" != "$target_path" ] && hybrid_index_exists "$path"; then
                echo "Remove inactive hybrid index before $target_kind/$target_dataset: $path"
                delete_hybrid_index "$dataset" "$kind"
            fi
        done
    done
...
run_hybrid_method() {
    ...
    ensure_only_target_hybrid_index "$dataset" "$method"
    if [ "$method" = "fhg" ]; then
...
echo "Truncated-normal mixed-alpha suite"
echo "Distribution: N(mu=0.5, sigma=0.1), truncated to [0.3, 0.7], seed=42"
...
if [ "$DRY_RUN" = "1" ]; then
    ...
    echo "All normal mixed-alpha experiments completed: $RESULT_ROOT"
fi
