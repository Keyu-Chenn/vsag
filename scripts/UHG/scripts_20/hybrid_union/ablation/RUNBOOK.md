...
```bash
ls "/tbase-project/vsag/scripts/UHG/results_20/main/nq/uhg/alpha_0.3.txt"
ls "/tbase-project/vsag/scripts/UHG/results_20/main/nq/uhg/alpha_0.7.txt"
```
...
| ef_search | `20 40 60 100 160 200` |
...
```bash
mkdir -p "/tbase-project/vsag/scripts/UHG/results_20/ablation/logs"

bash "/tbase-project/vsag/scripts/UHG/scripts_20/hybrid_union/ablation/run_ablation.sh" nq \
  2>&1 | tee "/tbase-project/vsag/scripts/UHG/results_20/ablation/logs/run_ablation_nq.log"
```

运行全部四个数据集：

```bash
bash "/tbase-project/vsag/scripts/UHG/scripts_20/hybrid_union/ablation/run_all.sh" \
  2>&1 | tee "/tbase-project/vsag/scripts/UHG/results_20/ablation/logs/run_ablation_all.log"
```

默认只产生两个新结果文件：

```text
/tbase-project/vsag/scripts/UHG/results_20/ablation/nq/uhg/alpha_0.3.txt
/tbase-project/vsag/scripts/UHG/results_20/ablation/nq/uhg/alpha_0.7.txt
```
...
```bash
python3 "/tbase-project/vsag/scripts/UHG/scripts_20/hybrid_union/ablation/plot_ablation.py" nq \
  2>&1 | tee "/tbase-project/vsag/scripts/UHG/results_20/ablation/logs/plot_ablation_nq.log"
```
...
两侧结果都是 `top-k=20`、全量 query、`build_alpha=0.5` 和
`hybrid_prune_scale=0.3`，避免快速验证结果或旧的 ablation `scale=0.5` 结果被误用。

输出：

```text
/tbase-project/vsag/scripts/UHG/results_20/ablation/plots/nq_ablation_recall_by_ef.png
/tbase-project/vsag/scripts/UHG/results_20/ablation/plots/nq_ablation_recall_by_ef.tsv
```
...
```bash
rg -c 'Recall:' "/tbase-project/vsag/scripts/UHG/results_20/ablation/nq/uhg"/alpha_{0.3,0.7}.txt
rg -n 'ERROR|NO_METRICS|Failed|Error:' "/tbase-project/vsag/scripts/UHG/results_20/ablation"
```
