...
DEFAULT_RESULT_DIR = Path("/tbase-project/vsag/scripts/UHG/results/quora_4methods")
DEFAULT_OUTPUT_PREFIX = DEFAULT_RESULT_DIR / "recall_qps_quora_4methods"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot Recall@100-QPS curves for Quora 4-method results."
    )
    parser.add_argument(
        "--result-dir",
        type=Path,
...
def plot(data, output_prefix, min_recall, min_visible_points):
    ...
    fig.suptitle(
        "QPS vs Recall@100 on Quora Dataset",
        fontsize=15,
        fontweight="bold",
        y=1.03,
    )
