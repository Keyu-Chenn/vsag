#!/usr/bin/env python3
"""Plot NQ and HotpotQA Recall-QPS curves for UHGS SINDI 1x/2x/3x.

Each dataset gets one PNG with alpha=0.3, 0.4, and 0.5 subplots. SINDI 1x
comes from the main experiment's method=auto results; auto routes all three
alphas to UHGS. This script only produces PNG images.
"""
...
SINDI_BK_RE = re.compile(r"\bsindi_bk=(?P<sindi_bk>\d+)")
...
SERIES = (
    ...
    Series(
        ...
        label="SINDI 2x",
        multiplier=2,
        result_subdir="uhgs_sindi_2x",
        color="#F58518",
        marker="s",
        required_method="method=uhgs",
    ),
    Series(
        key="sindi_3x",
        label="SINDI 3x",
        multiplier=3,
        result_subdir="uhgs_sindi_3x",
        color="#54A24B",
        marker="^",
        required_method="method=uhgs",
    ),
...
def parse_result(
    ...
    for line in path.read_text(errors="replace").splitlines():
        ...
        expected_sindi_bk = series.multiplier * ef_search
        sindi_bk_match = SINDI_BK_RE.search(line)
        if sindi_bk_match is not None:
            actual_sindi_bk = int(sindi_bk_match.group("sindi_bk"))
            if actual_sindi_bk != expected_sindi_bk:
                raise SystemExit(
                    f"Unexpected sindi_bk in {path}: ef_search={ef_search}, "
                    f"expected {expected_sindi_bk}, found {actual_sindi_bk}"
                )

        recall = float(metric_match.group("recall"))
        qps = float(metric_match.group("qps"))
        if not 0.0 <= recall <= 1.0 or qps <= 0.0:
            raise SystemExit(f"Invalid Recall/QPS point in {path}: {line}")
        if ef_search in seen_efs:
            raise SystemExit(f"Duplicate ef_search={ef_search} in {path}")
    ...
    for alpha in ALPHAS:
        ...
        for series in SERIES:
            ...
            if expected_efs is None:
                ...
                raise SystemExit(
                    f"Mismatched ef_search points for {dataset} alpha={alpha}: "
...
def plot_dataset(
    ...
    fig, axes = plt.subplots(1, len(ALPHAS), figsize=(13.2, 3.8), squeeze=False)
    ...
    fig.suptitle(
        f"{dataset_label} - UHGS SINDI Entry Candidates",
        fontsize=13,
        fontweight="bold",
    )
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    output_dir.mkdir(parents=True, exist_ok=True)
    output = output_dir / f"{dataset}_sindi_multiplier_qps_recall_log.png"
