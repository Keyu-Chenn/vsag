...
DEFAULT_BASE_RESULT_DIR = Path("/tbase-project/vsag/scripts/UHG/results/msmarco_4methods")
DEFAULT_UHGH_RESULT_DIR = Path("/tbase-project/vsag/scripts/UHG/results/msmarco_uhgh")
DEFAULT_OUTPUT_PREFIX = Path(
    "/tbase-project/vsag/scripts/UHG/results/msmarco_5methods/recall_qps_msmarco_5methods"
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot Recall@100-QPS curves for MSMARCO 4-method results plus UHGH."
    )
    parser.add_argument(
        "--base-result-dir",
        type=Path,
        ...
        help=(
            "Directory containing existing msmarco_4methods alpha_*.txt files. "
...
def plot(data, output_prefix, min_recall, min_visible_points):
    ...
    fig.suptitle(
        "QPS vs Recall@100 on MSMARCO Dataset",
        fontsize=15,
        fontweight="bold",
        y=1.03,
    )
