#!/usr/bin/env bash
# Run one truncated-normal mixed-alpha search job.
#
# Usage:
#   bash run_search.sh <dataset> <10|20|50|100|200> \
#     <hnsw|sindi|hnsw_sindi|fhg|uhg>
#
# This script never builds or deletes an index. run_all.sh owns the hybrid
# index lifecycle.
set -Eeuo pipefail
...
usage() {
    echo "Usage: $0 <dataset> <10|20|50|100|200> <hnsw|sindi|hnsw_sindi|fhg|uhg>" >&2
}
...
case "$K" in
    10)
        MIXED_ROOT="${MIXED_ROOT:-$EXPERIMENT_ROOT/data/gt_top200}"
    ...
    50)
        MIXED_ROOT="${MIXED_ROOT:-$EXPERIMENT_ROOT/data/gt_top200}"
        HNSW_POINTS_TEXT="${HNSW_POINTS_TEXT:-50 100 150 250 400 500}"
        SINDI_POINTS_TEXT="${SINDI_POINTS_TEXT:-500 750 1000 1500 2500 5000}"
        HNSW_SINDI_POINTS_TEXT="${HNSW_SINDI_POINTS_TEXT:-50 75 100 150 250}"
        FHG_POINTS_TEXT="${FHG_POINTS_TEXT:-50 100 150 250 500}"
        UHG_POINTS_TEXT="${UHG_POINTS_TEXT:-50 100 150 250 500}"
        ;;
    100)
        MIXED_ROOT="${MIXED_ROOT:-$EXPERIMENT_ROOT/data/gt_top200}"
        HNSW_POINTS_TEXT="${HNSW_POINTS_TEXT:-100 200 300 500 800 1000}"
        SINDI_POINTS_TEXT="${SINDI_POINTS_TEXT:-1000 1500 2000 3000 5000 10000}"
        HNSW_SINDI_POINTS_TEXT="${HNSW_SINDI_POINTS_TEXT:-100 150 200 300 500}"
        FHG_POINTS_TEXT="${FHG_POINTS_TEXT:-100 200 300 500 1000}"
        UHG_POINTS_TEXT="${UHG_POINTS_TEXT:-100 200 300 500 1000}"
        ;;
    200)
        MIXED_ROOT="${MIXED_ROOT:-$EXPERIMENT_ROOT/data/gt_top200}"
...
H5_FILE="$SHARED_DATA_DIR/hdf5/${DATASET}.hdf5"
...
FHG_GRAPH="$SHARED_DATA_DIR/fhg/${DATASET}_fhg_alpha_0_5.h5"
UHG_GRAPH="$SHARED_DATA_DIR/uhg/${DATASET}_uhg.h5"
...
result_is_complete() {
    ...
    [ -f "$OUT_FILE" ] || return 1
    grep -Fqx \
        "# experiment=mixed_alpha_zt dataset=$DATASET method=$METHOD recall_at=$K" \
        "$OUT_FILE" || return 1
    grep -Fqx \
        "# distribution=truncated_normal alpha_min=0.3 alpha_max=0.7 normal_mean=0.5 normal_stddev=0.1 seed=42" \
        "$OUT_FILE" || return 1
...
{
    echo "# experiment=mixed_alpha_zt dataset=$DATASET method=$METHOD recall_at=$K"
    echo "# distribution=truncated_normal alpha_min=0.3 alpha_max=0.7 normal_mean=0.5 normal_stddev=0.1 seed=42"
