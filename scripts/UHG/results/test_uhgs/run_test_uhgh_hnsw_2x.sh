#!/usr/bin/env bash
# Force UHGH at alpha=0.6/0.7 with twice as many dense-entry HNSW candidates:
#
#   dense_entry_bk        = 2 * ef_search
#   dense_entry_ef_search = 2 * ef_search
#
# The HNSW search width is increased together with the requested result count,
# so HNSW can actually return the requested number of entry points. Existing
# UHG hybrid indexes are loaded as-is; this script never rebuilds an index.
#
# Usage:
#   bash /tbase-project/vsag/scripts/UHG/results/test_uhgs/run_test_uhgh_hnsw_2x.sh
#   bash /tbase-project/vsag/scripts/UHG/results/test_uhgs/run_test_uhgh_hnsw_2x.sh nq
#
# Results:
#   <RESULT_DIR>/<dataset>/uhgh_hnsw_2x/alpha_<0.6|0.7>.txt
set -uo pipefail
...
METHOD="uhgh"
RESULT_VARIANT="uhgh_hnsw_2x"
BUILD_ALPHA="0.5"
HNSW_BK_MULTIPLIER=2
ALPHAS=(0.6 0.7)
...
for dataset in "${DATASETS[@]}"; do
    ...
    echo "  HNSW candidates: dense_entry_bk=${HNSW_BK_MULTIPLIER}*ef_search"
    echo "  HNSW width: dense_entry_ef_search=${HNSW_BK_MULTIPLIER}*ef_search"
    ...
    for alpha in "${ALPHAS[@]}"; do
        ...
        {
            echo "# UHGH with dense-entry HNSW candidates and search width set to 2x ef_search"
            ...
            echo "# dense_entry_bk=${HNSW_BK_MULTIPLIER}*ef_search dense_entry_ef_search=${HNSW_BK_MULTIPLIER}*ef_search"
        ...
        echo "--- $dataset $METHOD alpha=$alpha dense-entry HNSW=${HNSW_BK_MULTIPLIER}*ef_search ---"
        for ef in "${POINTS[@]}"; do
            dense_entry_bk=$((ef * HNSW_BK_MULTIPLIER))
            ...
            echo "$METHOD ef_search=$ef dense_entry_bk=$dense_entry_bk dense_entry_ef_search=$dense_entry_ef_search prune_scale=$HYBRID_PRUNE_SCALE threads=$THREADS $result" \
                | tee -a "$outfile"
        done
        echo "Done: $outfile"
    done
...
echo "UHGH dense-entry HNSW-2x experiments complete. Results: $RESULT_DIR"
