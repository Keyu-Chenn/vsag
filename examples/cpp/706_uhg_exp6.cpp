...
void
PrintGraphStatistics(const std::string& name, const std::vector<std::vector<int64_t>>& graph) {
    if (graph.empty()) {
...
PrintUsage(const char* program_name) {
    std::cout << "Usage: " << program_name << " <h5_file> [options]\n"
              ...
              << "\nExample:\n"
              << "  " << program_name << " nq.hdf5 -k 32 --alpha_step 0.1 --output nq_graph_refined.h5\n"
              << "  " << program_name
              << " nq.hdf5 --index_dir /tbase-project/vsag/scripts/UHG/data/index --threads 16\n"
              << std::endl;
}
...
main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "\n========== Hybrid Graph Generation with Refine ==========\n";
        ...
        int num_process =
            (config.num_points > 0) ? std::min(config.num_points, static_cast<int>(num_train))
                                    : static_cast<int>(num_train);
        ...
        if (need_build_sparse) {
            ...
            out_stream.close();
            if (!serialize_result.has_value()) {
                std::cerr << "Failed to save SINDI: " << serialize_result.error().message
                          << std::endl;
                return -1;
            }
            ...
            in_stream.close();
            if (!deserialize_result.has_value()) {
                std::cerr << "Failed to load SINDI: " << deserialize_result.error().message
                          << std::endl;
                return -1;
            }
...
#pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < num_process; i++) {
            ...
            total_dense_us.fetch_add(
                std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count(),
                std::memory_order_relaxed);
            total_sparse_us.fetch_add(
            ...
            for (float alpha : alpha_values) {
                ...
                int actual_k = std::min(config.k, static_cast<int>(candidates.size()));
...
#pragma omp critical
                {
                    ...
                    std::cout << "  [" << processed_count << "/" << num_process << "] " << elapsed
                              << "s, eta:" << eta << "s" << std::endl;
                }
            }
        }
        ...
        std::cout << "\nGeneration time: "
                  << std::chrono::duration<double>(gen_end - gen_start).count() << "s" << std::endl;
        std::cout << "  dense search total (thread-sum): " << total_dense_us.load() / 1e6 << "s"
                  << std::endl;
