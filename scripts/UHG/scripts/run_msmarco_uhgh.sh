#!/usr/bin/env bash
# MSMARCO UHGH experiment with 707_uhg_exp7, Recall@100.
#
# Usage:
#   bash scripts/UHG/scripts/run_msmarco_uhgh.sh          # run all alpha values
#   bash scripts/UHG/scripts/run_msmarco_uhgh.sh 0.5      # run one alpha
#   NUM_QUERIES=100 bash scripts/UHG/scripts/run_msmarco_uhgh.sh 0.5
set -euo pipefail

DATASET="${DATASET:-/tbase-project/vsag/scripts/UHG/data/hdf5/msmarco.hdf5}"
GT_DIR="${GT_DIR:-/tbase-project/vsag/scripts/UHG/data/ground_truth/msmarco}"
DENSE_INDEX_DIR="${DENSE_INDEX_DIR:-/tbase-project/vsag/scripts/UHG/data/index}"
HYBRID_INDEX_DIR="${HYBRID_INDEX_DIR:-/tbase-project/vsag/scripts/UHG/data/index_refined_msmarco}"
HNSW_INDEX_PATH="${HNSW_INDEX_PATH:-$DENSE_INDEX_DIR/701_msmarco_dense_hnsw.index}"
HYBRID_INDEX_PATH="${HYBRID_INDEX_PATH:-$HYBRID_INDEX_DIR/703_msmarco_hybrid_index.index}"
RESULT_DIR="${RESULT_DIR:-/tbase-project/vsag/scripts/UHG/results/msmarco_uhgh}"
K="${K:-100}"
...
for ALPHA in "${ALPHAS[@]}"; do
    ...
    {
        echo "# MSMARCO UHGH experiment, alpha=$ALPHA, k=$K, Recall@$K"
    ...
    echo "=== MSMARCO UHGH Recall@$K alpha=$ALPHA ==="
