#!/usr/bin/env bash
# MSMARCO refined UHG/UHGS experiment, Recall@100.
#
# Usage:
#   bash scripts/UHG/scripts/run_msmarco_refined_uhg.sh        # run all alpha values
#   bash scripts/UHG/scripts/run_msmarco_refined_uhg.sh 0.5    # run one alpha
#
# The first run builds scripts/UHG/data/index_refined_msmarco/703_msmarco_*.index
# if those files do not exist. Make sure HybridIndex is compiled with
# scripts/UHG/data/uhg/msmarco_uhg_refined.h5 as the precomputed graph path.
set -euo pipefail
...
REFINED_INDEX_DIR="/tbase-project/vsag/scripts/UHG/data/index_refined_msmarco"
RESULT_DIR="/tbase-project/vsag/scripts/UHG/results/msmarco_refined_uhg"
K=100
...
mkdir -p "$REFINED_INDEX_DIR" "$RESULT_DIR"

for ALPHA in "${ALPHAS[@]}"; do
    OUTFILE="$RESULT_DIR/alpha_${ALPHA}.txt"
    {
        echo "# MSMARCO refined UHG/UHGS experiment, alpha=$ALPHA, k=$K, Recall@$K"
        echo "# hnsw_sindi index_dir=$BASE_INDEX_DIR"
        echo "# refined uhg/uhgs index_dir=$REFINED_INDEX_DIR"
        echo "# num_queries=$NUM_QUERIES"
    } > "$OUTFILE"

    echo "=== MSMARCO Recall@$K alpha=$ALPHA ==="
