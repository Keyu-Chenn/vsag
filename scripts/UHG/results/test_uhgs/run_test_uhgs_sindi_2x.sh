#!/usr/bin/env bash
# Force UHGS at all five main-experiment alphas with twice as many SINDI entry
# candidates:
#
#   sindi_bk = 2 * ef_search
#
# All other search settings match run_test_uhgs.sh. Existing UHG hybrid indexes
# are loaded as-is; this script never rebuilds an index.
#
# Usage:
#   bash /tbase-project/vsag/scripts/UHG/results/test_uhgs/run_test_uhgs_sindi_2x.sh
#   bash /tbase-project/vsag/scripts/UHG/results/test_uhgs/run_test_uhgs_sindi_2x.sh nq
#   DATASETS_TEXT="nq hotpotqa" POINTS_TEXT="100 300" \
#     bash /tbase-project/vsag/scripts/UHG/results/test_uhgs/run_test_uhgs_sindi_2x.sh
#
# Results:
#   <RESULT_DIR>/<dataset>/uhgs_sindi_2x/alpha_<0.3|0.4|0.5|0.6|0.7>.txt
set -uo pipefail
...
METHOD="uhgs"
RESULT_VARIANT="uhgs_sindi_2x"
BUILD_ALPHA="0.5"
SINDI_BK_MULTIPLIER=2
...
if [ "$#" -ge 2 ]; then
    echo "Unexpected argument: this experiment uses the five main-experiment alphas"
    exit 1
fi
...
for dataset in "${DATASETS[@]}"; do
    ...
    for alpha in "${ALPHAS[@]}"; do
        ...
        {
            echo "# UHGS with SINDI entry candidates set to 2x ef_search"
            ...
            echo "# main auto comparison=$main_auto_result (auto routes alpha<=0.5 to UHGS and alpha>0.5 to UHGH)"
...
echo "UHGS SINDI-2x all-alpha experiments complete. Results: $RESULT_DIR"
