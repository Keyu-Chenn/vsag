#!/usr/bin/env bash
# NQ 4-method experiment, Recall@100.
#
# Usage:
#   bash scripts/UHG/scripts/run_nq_4methods.sh        # run all alpha values
#   bash scripts/UHG/scripts/run_nq_4methods.sh 0.3    # run one alpha
set -euo pipefail

DATASET="/tbase-project/vsag/scripts/UHG/data/hdf5/nq.hdf5"
GT_DIR="/tbase-project/vsag/scripts/UHG/data/ground_truth/nq"
BASE_INDEX_DIR="/tbase-project/vsag/scripts/UHG/data/index"
REFINED_INDEX_DIR="/tbase-project/vsag/scripts/UHG/data/index_refined"
RESULT_DIR="/tbase-project/vsag/scripts/UHG/results/nq_4methods"
K=100
...
for ALPHA in "${ALPHAS[@]}"; do
    ...
    echo "--- FHG ---"
    cp "$BASE_INDEX_DIR/703_nq_fhg_k64_hybrid_index.index" "$BASE_INDEX_DIR/703_nq_hybrid_index.index"
