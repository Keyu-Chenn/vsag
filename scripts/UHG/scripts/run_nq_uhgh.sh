#!/usr/bin/env bash
# NQ UHGH experiment with 707_uhg_exp7, Recall@100.
#
# Usage:
#   bash scripts/UHG/scripts/run_nq_uhgh.sh          # run all alpha values
#   bash scripts/UHG/scripts/run_nq_uhgh.sh 0.5      # run one alpha
#   NUM_QUERIES=100 bash scripts/UHG/scripts/run_nq_uhgh.sh 0.5
set -euo pipefail

DATASET="${DATASET:-/tbase-project/vsag/scripts/UHG/data/hdf5/nq.hdf5}"
GT_DIR="${GT_DIR:-/tbase-project/vsag/scripts/UHG/data/ground_truth/nq}"
DENSE_INDEX_DIR="${DENSE_INDEX_DIR:-/tbase-project/vsag/scripts/UHG/data/index}"
HYBRID_INDEX_DIR="${HYBRID_INDEX_DIR:-/tbase-project/vsag/scripts/UHG/data/index_refined}"
HNSW_INDEX_PATH="${HNSW_INDEX_PATH:-$DENSE_INDEX_DIR/701_nq_dense_hnsw.index}"
HYBRID_INDEX_PATH="${HYBRID_INDEX_PATH:-$HYBRID_INDEX_DIR/703_nq_hybrid_index.index}"
RESULT_DIR="${RESULT_DIR:-/tbase-project/vsag/scripts/UHG/results/nq_uhgh}"
K="${K:-100}"
...
for ALPHA in "${ALPHAS[@]}"; do
    ...
    {
        echo "# NQ UHGH experiment, alpha=$ALPHA, k=$K, Recall@$K"
    ...
    echo "=== NQ UHGH Recall@$K alpha=$ALPHA ==="
