...
RESULT_ROOT="${RESULT_ROOT:-$REPO_ROOT/scripts/UHG/results_50}"
DATASET="${1:-nq}"
...
echo "[$(date '+%F %T')] START $DATASET ablation (Recall@50)"
...
echo "[$(date '+%F %T')] START $DATASET test_prune (Recall@50)"
...
echo "[$(date '+%F %T')] DONE $DATASET four Recall@50 experiments"
